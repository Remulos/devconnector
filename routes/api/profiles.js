...
// Load profiles model to use in mongoose
const Profile = require('../../models/Profile');
// Load users model to use in mongoose
...
router.post(
	...
	(req, res) => {
		...
		// Get fields (see the ./models/Profile for the model)
		const profileFields = {};
		...
		Profile.findOne({ user: req.user.id }).then(profile => {
			if (profile) {
				// UPDATE - if a profile exists then this is an update and not a new profile
	...
	(req, res) => {
		...
		Profile.findOne({ user: req.user.id })
			.then(profile => {
				...
				if (removeIndex == -1) {
					errors.noexptodelete =
						'There is no experience of that id to delete';
					res.status(404).json(errors);
				} else {
					// Splice out of array
	...
	(req, res) => {
		...
		Profile.findOne({ user: req.user.id })
			.then(profile => {
				...
				// Check if the edu_id exists
				if (removeIndex == -1) {
					errors.noedutodelete =
						'There is no education of that id to delete';
					res.status(404).json(errors);
				} else {
					// Splice out of array
